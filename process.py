...
Behavior:
 ...
 - run Docker scan (retries/backoff) - this automatically generates posters
"""
...
class FileProcessor:
    ...
    def _run_scan(self, final_path: Path, max_retries: int = 3, initial_backoff: float = 1.0) -> bool:
        ...
        The scan-video command automatically generates poster images, so no separate
        poster generation step is needed.
        """
        ...
        logger.info("Running scan command (will auto-generate poster): %s", cmd_str)
        if self.dry_run:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if res.returncode == 0:
                    logger.info("Scan completed successfully for %s (poster auto-generated)", final_path.name)
                    return True
                else:
    ...
    def process(self, src_path: Path) -> bool:
        ...
        try:
            ...
            # Note: scan-video automatically generates the poster, so no separate step needed
            scan_ok = self._run_scan(final)
            if not scan_ok:
                logger.warning("Scan reported failure for %s", final)
                return False

            logger.info("Finished processing %s (poster auto-generated by scan)", final.name)
            return True
        finally:
            if workdir:
